...
ssize_t
timerd_read(int s, char *buf, size_t len, int sec, int flags)
{
    ssize_t r = -1;
...
ssize_t
timerd_write(int s, char *buf, size_t len, int sec)
{
    ssize_t r = -1;
...
int
addr_comp(struct bin_addr *a1, struct bin_addr *a2, int mask)
{
    int    ret = -1;
...
int
lookup_tbl(struct socks_req *req)
{
    int    i, match, error;
...
int
resolv_host(struct bin_addr *addr, u_int16_t port, struct host_info *info)
{
...
*/
int
log_request(int v, struct socks_req *req, struct req_host_info *info)
{
    struct  sockaddr_storage ss;
...
int
bind_sock(int s, struct socks_req *req, struct addrinfo *ai)
{
    /*
...
int
do_bind(int s, struct addrinfo *ai, u_int16_t p)
{
...
int
socks_rep(int s, int ver, int code, struct sockaddr *addr)
{
    struct sockaddr_in  *sa;
    struct sockaddr_in6 *sa6;
    u_char     buf[512];
...
*/
int
proxy_connect(int ver, struct socks_req *req)
{
    int s;
...
int
connect_to_socks(int ver, struct socks_req *req)
{
    int     cs;
    int     r, len = 0;
    struct  addrinfo hints, *res, *res0;
...
int
proxy_reply(int v, int cs, int ss, int req)
{
    int     r, c, len;
    u_char  buf[512];
    u_char  rep[512];
