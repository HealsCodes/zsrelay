...
void
add_entry(struct rtbl *r, struct rtbl *t, int ind)
{
    if (ind >= MAX_ROUTE)
      /* error in add_entry */
      return;
...
void
parse_err(int sev, int line, char *msg)
{
    msg_out(sev, "%s: line %d: %s\n", CONFIG, line, msg);
}

int
str_to_addr(char *addr, struct bin_addr *dest)
{
    char     *q;
    int      len, i, c;
...
*/
int
readpasswd(FILE *fp, int ind, char *user, int ulen, char *pass, int plen)
{
    char     buf[MAXLINE];
    char     *p, *tok;
    int      len;
